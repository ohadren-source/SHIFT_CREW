#!/usr/bin/env python3
"""
Production seed script — run with production DATABASE_URL
Usage: DATABASE_URL="postgresql://..." python seed-production.py
"""
...
if not DATABASE_URL:
    ...
    print("Usage: DATABASE_URL='postgresql://...' python seed-production.py")
    sys.exit(1)
...
from models import Base, Role, Facility, Shift, Task
...
def seed_database():
    ...
    print(f"✓ Created/verified facility: {facility.name}")
    ...
    db.commit()
    print(f"✓ Created/verified {len(shifts)} shifts")
    ...
    db.commit()
    print(f"✓ Created {task_count} new tasks")
